...
function duplicateTemplate(template) {
  // Create a modal to get a new title for the duplicated template
  const modal = document.createElement('div');
  modal.className = 'modal fade';
  modal.id = 'duplicateTemplateModal';
  modal.setAttribute('tabindex', '-1');
  modal.setAttribute('role', 'dialog');
  modal.setAttribute('aria-labelledby', 'duplicateTemplateModalLabel');
  modal.setAttribute('aria-hidden', 'true');
  ...
  modal.innerHTML = `
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="duplicateTemplateModalLabel">Duplicate Template</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form id="duplicateForm">
            <div class="form-group mb-3">
              <label for="newTemplateTitle">New Title</label>
              <input type="text" class="form-control" id="newTemplateTitle" 
                value="${template.title} (Copy)" required>
            </div>
            <div class="form-group mb-3">
              <label for="newTemplateDescription">Description</label>
              <textarea class="form-control" id="newTemplateDescription" rows="3">${template.description}</textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
  ...
  document.getElementById('confirmDuplicateBtn').addEventListener('click', async () => {
    ...
    if (!title) {
      alert('Please enter a title for the new template');
      return;
    }
    
    // Show loading state
    const duplicateBtn = document.getElementById('confirmDuplicateBtn');
    duplicateBtn.disabled = true;
    duplicateBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Duplicating...';
    ...
    try {
      ...
      const response = await fetch(`/api/documents/${template._id}/duplicate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      showAlert('danger', 'Failed to duplicate template: ' + error.message);
    } finally {
      // Reset button state
...
function uploadTemplate() {
  ...
  .then(response => {
    if (!response.ok) {
      if (response.status === 403) {
        throw new Error('Permission denied. Your account may not have the required permissions.');
  ...
  .catch(error => {
    ...
    showAlert('danger', 'Failed to upload template: ' + error.message);
  })
  .finally(() => {
    // Reset button state
